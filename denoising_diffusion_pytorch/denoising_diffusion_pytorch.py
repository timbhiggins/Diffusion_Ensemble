...
set_seed(42)  # Use the same seed across all processes
...
class GaussianDiffusion(Module):
    def __init__(
        ...
        *,
        ...
        ddim_sampling_eta, #default is 0
        auto_normalize = True,
    ...
    def ddim_sample(self, shape, x_cond, return_all_timesteps = False):
        ...
        sigma = 1
        img = sigma*torch.randn(shape, device = device)
        imgs = [img]
...
class Trainer:
    def __init__(
        ...
        *,
        ...
        num_samples = 25,
        results_folder = '/glade/work/timothyh/Diffusion/Forecasting/denoising-diffusion-pytorch/best_models/',
        amp = False,
        ...
        self.lead = lead
        self.sample = diffusion_model.sample
    ...
    def save(self, milestone):
        ...
        torch.save(data, str(self.results_folder / f'model-{milestone}{self.run_name}.pt'))

    def load(self, milestone):
        accelerator = self.accelerator
        device = accelerator.device
        
        data = torch.load(str(self.results_folder / f'model-{milestone}{self.run_name}.pt'), map_location=device)
    ...
    def train(self):
        ...
        maxivt = 15.220598
        minivt = -1.0300009
        dataX = xr.open_dataset('/glade/derecho/scratch/timothyh/data/diffusion_forecasts/processed/lead_'+str(self.lead)+'/val.nc').forecast
        datay = xr.open_dataset('/glade/derecho/scratch/timothyh/data/diffusion_forecasts/processed/lead_'+str(self.lead)+'/val.nc').analysis
            ...
            while self.step < self.train_num_steps:
                ...
                if (self.step%10) == 0:
                    if self.do_wandb:
                        ...
                        #folder_path = Path("/glade/derecho/scratch/timothyh/diffusion/diff_results/")       
                        #all_denoised = torch.zeros([20,20,1,128,128]).to(device)
                        #all_denoised[0,:,:,:,:] = X[0:20,:]
                        #sampled_images = self.sample(all_denoised[0,:,:,:,:], batch_size = 20, return_all_timesteps = False)
                        ...
                        #Y = np.array(datay)
                        #rmse = climpred.metrics._rmse(xr.DataArray(np.mean(all_denoised[:,:,0,:,:],axis=0)),xr.DataArray(Y[:20,:,:]),dim=["dim_0"])
                        #spread = np.mean(np.std(all_denoised[:,:20,0,:,:],axis=0))
                        #spreadskill = np.mean(spread)/np.mean(rmse)
                        #self.accelerator.log({'spreadskill': spreadskill.item()})
